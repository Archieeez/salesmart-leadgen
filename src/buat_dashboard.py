...
<li><b>Yang dikerjakan sambil menunggu, dan batasnya.</b> Jawaban BPS 3 September memisahkan dua hal: memakai <i>nama dan alamat</i> tidak butuh izin tertulis; yang dilarang adalah <b>reproduksi atau penerbitan kembali publikasinya</b>. Jadi pengekstrak dibangun (<code>src/bps_ekstrak.py</code>, 35.129 baris &rarr; 235 kandidat prioritas), sementara <b>tidak satu pun baris itu terbit di repo ini</b>. PDF-nya sendiri tidak pernah di-commit.</li>
<li><b>Garisnya dijalankan mesin, bukan diingat orang.</b> <code>data/bps.db</code> dan <code>data/leads.db</code> keduanya di luar git &mdash; berkas biner tidak bisa disaring sebagian. Tiap query yang menulis berkas publik wajib lewat <code>publik.klausa()</code>, <code>src/uji_gerbang.py</code> menolak query baru yang lupa memakainya, dan <code>src/publik.py</code> memeriksa berkas yang <i>sudah</i> ditulis. Lihat <code>CATATAN_SUMBER_DATA.md</code>.</li>
