...
def blok_agen():
    ...
    try:
        st = ag.status(con)
        ev = ag.denyut(con, batas=12)
    finally:
        con.close()

    kelas = "jalan" if st["aktif"] else "diam"
    judul = "sedang bekerja" if st["aktif"] else "menganggur"
    sejak = ag.usia_manusia(st["detik_sejak"])

    h = [f'<div class="agen {kelas}">']
    h.append('<div class="agen-kepala"><span class="nadi"></span>'
             f'<div><div class="agen-judul">Agen &middot; {judul}</div>'
             f'<div class="agen-kini">{esc(st["kegiatan"])}</div></div>'
             f'<div class="agen-sejak">{esc(sejak)}</div></div>')
    ...
    if ev:
        ...
        for waktu, jenis, teks in ev:
            h.append(f'<li><span class="jam">'
                     f'{waktu.strftime("%H:%M")}</span>'
    ...
    h.append('<div class="agen-kaki">Denyut ini jejak pekerjaan yang '
             ...
             '&mdash; ia bekerja waktu sesi dibuka, dan panel ini menulis '
             '&ldquo;menganggur&rdquo; kalau memang sedang menganggur.</div>')
    h.append('</div>')
    return "\n".join(h)
